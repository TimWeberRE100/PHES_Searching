...
def size_of_document(document, num_tabs=1):
    ...
    size = (
        len_of_attrib(document.attrib) + 7 + num_tabs + 2 * tag_length
    )  # tag name is bounded above by 10
...
def get_site(task):
    ...
    return (
        type
        + ns
        + str(abs(int(task[-1]))).zfill(2)
        + "_"
        + ew
        + str(abs(int(task[-2]))).zfill(3)
    )
...
def main(path_to_tasks_file, path_to_final_output_classes, output_path="."):
    ...
    for task in tqdm(tasks):
        ...
        try:
            ...
            print("FAILED:", total_csv_file_name)
            exit()
    ...
    for j, size in enumerate(tqdm(sizes)):
        ...
        for task in tqdm(tasks, leave=False):
            site = get_site(task)
            total_csv_file_name = (
                final_output_classes_path / site / (site + "_total.csv")
            )
            ...
            if (list(total_csv_df.iloc[j])[-1]) != 0:
                ...
                csv_file_name = (
                    final_output_classes_path / site / (site + "_" + size + ".csv")
                )
                try:
                    ...
                    print("FAILED:", csv_file_name)
                # Create summary kml
                # Read in kml
                kml_file_name = (
                    final_output_classes_path / site / (site + "_" + size + ".kml")
                )
                try:
                    tree = ET.parse(kml_file_name)
                except:
                    print("FAILED:", kml_file_name)
                root = tree.getroot()
                ...
                if total_size > FILESIZE * 1000000:
                    ...
                    grouped_file_path = kmls_folder / (
                        size + "_summary_" + str(kml_file_no) + ".kml"
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Summarise output of final_output_classes"
    )
    parser.add_argument(
        "--task_file_path",
        metavar="task_file_path",
        required=True,
        help="the path to task file (from working directory)",
    )
    parser.add_argument(
        "--final_output_classes_path",
        metavar="final_output_classes_path",
        required=True,
    ...
    main(
        path_to_tasks_file=args.task_file_path,
        path_to_final_output_classes=args.final_output_classes_path,
